...
class Document:
	def __init__(self, title, tables, currDir, transposedDir):
		...
		strings.append('<link rel="shortcut icon" href="http://www.browndailyherald.com/wp-content/uploads/2012/11/favicon.jpg"/>')
...
class CrossTab:
	def __init__(self, title, xLabel, yLabel, data):
		...
		if self.chisquare[1] != 0 and not (np.isnan(self.chisquare[1])):
...
class PollData:
	...
	def parseEntry(self, row, question):
		...
		if (len(answers2) > 1):
			if not question.choose_multiple:
				for answer in answers:
					if entry == answer:
						return map(lambda x: question.listOptions.index(x), [answer])
		if (len(answers2) < 1):
			return None
		return answers2
